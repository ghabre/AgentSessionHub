...
$tmpWin  = Join-Path $env:TEMP 'recent-claude'
...
$clickStateWsl = '/tmp/recent-claude-click.state'
$clickBinds = @"
...
$cacheFile = Join-Path $env:TEMP 'recent-claude-cache.json'
$cache = @{}
...
function Get-Sessions {
    ...
    wsl.exe -d $distro -- bash $scanShWsl |
        ForEach-Object {
            ...
            "{0}{1} - {2} - {3} ago [..-{4}]`t{5}`t{6}`t{7}" -f $tag, $folder, $title, $age, $shortId, $id, $cwd, $winPath
        }
...
function Invoke-Codex($id, $cwd, $transcript) {
...
while ($true) {
    ...
    foreach ($row in $picked) {
        ...
        if ($tool -eq 'codex')  { Invoke-Codex $id $cwd $tr; continue }  # resume in codex
